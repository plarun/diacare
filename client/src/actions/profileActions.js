...
export const isValidUser = (useras, handle) => (dispatch) => {
	return axios
		.get(`api/${useras}/isValidUser/${handle}`)
		.then((res) => {
			console.log(res)
			return res
		})
...
export const addGlucoseData = (handle, glucoseData, history) => (dispatch) => {
	...
	axios
		.post("/api/patient/chart", { handle, glucoseData })
		.then((res) =>
			dispatch({
				type: GET_PROFILE,
				payload: res.data,
...
export const requestDoctor = (doctor, _id, handle, mtype) => (dispatch) => {
	axios
		.post("/api/patient/request", { doctor, _id, handle, mtype })
		.then((res) =>
			dispatch({
				type: GET_PROFILE,
				payload: res.data,
...
export const rejectPatient = (doctor, patient) => (dispatch) => {
	axios
		.post("api/doctor/reject", { doctor, patient })
		.then((res) =>
			dispatch({
				type: GET_PROFILE,
				payload: res.data,
...
export const removeMessage = (doctorHandle, msg_id) => (dispatch) => {
	axios
		.post("api/doctor/removeMsg", { doctorHandle, msg_id })
		.then((res) =>
			dispatch({
				type: GET_PROFILE,
				payload: res.data,
...
export const subscribeDoctor = (handle, doctor) => (dispatch) => {
	axios
		.post("/api/patient/subscribe", { handle, doctor })
		.then((res) =>
			dispatch({
				type: GET_PROFILE,
				payload: res.data,
...
// unsubscribe doctor
export const unsubscribeDoctor = (patient_handle, doctor_handle) => (
	dispatch
) => {
	axios
		.post("/api/patient/unsubscribe", { patient_handle, doctor_handle })
		.then((res) =>
			dispatch({
				type: GET_PROFILE,
				payload: res.data,
...
export const getMyPatients = (handle) => (dispatch) => {
	...
	console.log(handle)
	axios
		.get(`/api/doctor/myPatient/${handle}`)
		.then((res) => {
			console.log(res.data)
			dispatch({
				type: GET_PROFILES,
